...
def picture_block(image_dict):
    ...
    while True:
        ...
        if choice.upper() == "N":
            ...
            print("下载图片了")
            data_id = choice
            data_title = image_dict[choice][0]
            message = "{}-{}".format(choice, data_title)
            print(message)


def nba_block(nba_dict):
    for key, value in nba_dict.items():
        print(key, value["title"])
    while True:
        choice = input("选择下载NBA视频序号n/N：")
        if choice.upper() == "N":
            break
        else:
            print("下载视频了")
            data_title = nba_dict[choice]["title"]
            data_url = nba_dict[choice]["url"]
            message = "{}-{}".format(data_title, data_url)
            print(message)


def short_video_block(video_dict):
    ...
    while True:
        ...
        if choice.upper() == "N":
            ...
            print("下载视频了")
            data_title = video_dict[choice]["title"]
            data_url = video_dict[choice]["url"]
            message = "{}-{}".format(data_title, data_url)
            print(message)
...
while True:
    ...
    if choice_block.upper() == "Q":
        break
    elif choice_block not in data_dict:
        continue
    else:
        choice_data = func_dict[choice_block]
        if not choice_data:
            print("输入有误")
        else:
            choice_data(data_dict[choice_block])
